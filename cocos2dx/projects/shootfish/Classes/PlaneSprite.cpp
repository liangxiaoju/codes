...
void PlaneSprite::startShoot() {
    schedule(schedule_selector(PlaneSprite::shootOnce), 0.20, kRepeatForever, 0);
}
...
void PlaneSprite::shootOnce(float dt) {
    ...
    if (mSuperPower == 0) {
        ...
        bullet1->setPosition(this->getPosition() + Vec2(-7, 10));
        bullet2->setPosition(this->getPosition() + Vec2(7, 10));
        GameLayer::getInstance()->addChild(bullet1, 0, "bullet");
