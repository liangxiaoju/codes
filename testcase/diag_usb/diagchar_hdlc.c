...
int diag_usb_encode(void *in, int in_len, void *out, int out_len)
{
	...
	send.pkt = in;
	send.last = (void *)(in + in_len - 1);
	send.terminate = 1;

	enc.dest = out;
	enc.dest_last = (void *)(out+ out_len - 1);
	enc.crc = 0;

	return diag_hdlc_encode(&send, &enc);
}
...
int diag_usb_decode(void *in, int in_len, void *out, int out_len)
{
	...
	hdlc.dest_ptr = out;
	hdlc.dest_size = out_len;
	hdlc.src_ptr = in;
	hdlc.src_size = in_len;
	hdlc.src_idx = 0;
	hdlc.dest_idx = 0;
	hdlc.escaping = 0;
	...
	if (ret) {
		...
		if (crc_chk) {
			/* CRC check failed. */
			printf("diag: In %s, bad CRC. Dropping packet\n",
								__func__);
			return -1;
		}
	}

	return hdlc.dest_idx;
}
