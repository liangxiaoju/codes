/**
 * klog.c - a log channel between kernel and userspace
 *
 * use to print device driver's error message to userspace,
 * so that them can be logged to file.
 *
...
/**
 * klog_fam - generic netlink family
 */
...
/**
 * klog_msg_event - multicast the @msg to @group
 */
...
/**
 * klog_proc - use in eng version for debug.
 */
...
/**
 * register klog family and its mutilcast group
 */
...
#ifdef BUILD_ENG_VERSION
	proc = proc_create("klog", 0600, NULL, &klog_proc_fops);
	if (!proc)
		goto out;
#endif
