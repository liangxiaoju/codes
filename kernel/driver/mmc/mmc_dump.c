/**
 * mmc_dump.c - dump mmc transfer cmd
 *
 * if emmc error happened, dump the last 10 emmc transfer record.
 *
...
/**
 * struct mmc_record - use to record mmc_request
 */
struct mmc_record {
...
/**
 * struct mem_pool - our own memory pool to speed up
 */
struct mem_pool {
	void *va_addr;
	int max_nr;
...
/**
 * struct mmc_dump - our private data
 *
 * @kthread: kernel thread used to handle each mmc_request
 * @queue: mmc_record list
 * @user_queue: for userspace
 *
 */
struct mmc_dump {
...
/**
 * grab_next_buffer - get memory from mem_pool
 * @dump: mmc_dump
 * @for_user: get from user_pool if 1
 */
...
/**
 * init_record_from_mrq - copy content from mmc_request to mmc_record
 */
...
/**
 * init_record_from_old - copy mmc_record
 */
...
/**
 * mmc_dump_event - mmc dump hook
 *
 * @host: which mmc_host the mrq belong to
 * @mrq: current mmc_request
 *
 * call from mmc_request_done()
 *
...
/**
 * dump_record - dump the record out
 *
...
/**
 * filter out the unnecessary error
 */
...
/**
 * handle_records - the actual handler
 */
...
#ifdef BUILD_ENG_VERSION
/**
 * /proc/mmc - use to dump out all the emmc record
 *
 * use in eng version only.
 */
...
/**
 * init_pool - build up our own memory pool
 */
...
#ifdef BUILD_ENG_VERSION
	dump->proc = proc_create("mmc", S_IRUSR|S_IWUSR, NULL, &mmc_dump_proc_fops);
	if (!dump->proc)
		goto destroy_pool;
#endif
...
MODULE_AUTHOR("liangxiaoju <liangxiaoju@gmail.com>");
