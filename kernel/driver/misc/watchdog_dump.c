/**
 * watchdog_dump.c - dump the cpu regs when watchdog timeout happened
 *
 * Used under qualcomm platform only.
 *
...
/**
 * define the reserve memory range.
 */
...
/**
 * /proc/last_cpu: cpu regs.
 * /proc/watchdog: debug node used to trigger watchdog timeout, only in eng version.
 */
static int __init init_wdogdump_proc(void)
{
	proc_create("last_cpu", S_IRUSR, NULL, &wdogdump_proc_fops);
...
/**
 * use to check if the poweron reason is watchdog timeout.
 * call from poweron_reason.c
 */
...
/**
 * msm-imem-mem_dump_table is the phys address for dump table,
 * and all the msm_dump_data_ids's address can be found in it.
 */
 ...
 *
 * save the last wdog dump to buffer,
 * and dump it when user cat /proc/lastcpu.
 */
...
/**
 * The main idea is to change qualcomm's wdog dump address to our own buffer,
 * so we can get the dump message out.
 */
...
/**
 * wdog_dump_memblock_reserve - reserve the memory for wdog_dump
 *
