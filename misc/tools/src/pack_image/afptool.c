...
int pack_update(const char* srcdir, const char* dstfile,unsigned int debug) {
	struct update_header header;
	FILE *fp = NULL;
	int i;
	char buf[PATH_MAX];
	...
	if(debug == 1){
		snprintf(buf, sizeof(buf), "%s/%s", srcdir, "parameter-debug");
		if (parse_parameter(buf))
			return -1;
		snprintf(buf, sizeof(buf), "%s/%s", srcdir, "package-file-debug");
		if (get_packages(buf))
			return -1;
	}else{
...
void usage(const char *appname) {
	...
	printf("USAGE:\n"
			...
			"\t%s -pack xxx update.img\tPack files  -debug\n"
...
int main(int argc, char** argv) {
	...
	if (strcmp(argv[1], "-pack") == 0 && argc == 4) {
		if (pack_update(argv[2], argv[3],0) == 0)
			printf("Pack OK!\n");
		else
			printf("Pack failed\n");
	} else 	if (strcmp(argv[1], "-pack") == 0 && argc == 5 && strcmp(argv[4], "-debug") == 0 ) {
		if (pack_update(argv[2], argv[3],1) == 0)
			printf("Pack OK!\n");
		else
			printf("Pack failed\n");
