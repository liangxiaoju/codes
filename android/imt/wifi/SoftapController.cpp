...
#define LOG_TAG "SoftapController-imt"
...
extern "C" int get_wifi_hostapd(char *hostapd, char *hostapd_prop);
...
int SoftapController::startSoftap() {
    char hostapd_name[PROPERTY_VALUE_MAX];
    char hostapd_prop_name[PROPERTY_KEY_MAX];
    ...
    if (get_wifi_hostapd(hostapd_name, hostapd_prop_name) < 0) {
        strcpy(hostapd_name, HOSTAPD_NAME);
        strcpy(hostapd_prop_name, HOSTAPD_PROP_NAME);
    }
    ...
    sched_yield();
    while (count-- > 0) {
        if (property_get(hostapd_prop_name, hostapd_status, NULL)) {
    ...
    ALOGD("%s %s %s", hostapd_name, hostapd_prop_name, hostapd_status);
...
int SoftapController::stopSoftap() {
    ...
    while (count-- > 0) {
        if (property_get(hostapd_prop_name, hostapd_status, NULL)) {
            if (strcmp(hostapd_status, "stopped") == 0) {
                break;
            }
        }
        usleep(100000);
    ...
    if (count <= 0) {
        ALOGD("SoftAP failed to stopped");
        return ResponseCode::ServiceStopFailed;
    } else {
        mPid = 0;
