...
static struct wifi_info wifi_infos[] = {
    {
        .name = "rtl8188eu",
        .modpath = "8188eu.ko",
        .modarg = "ifname=wlan0 if2name=p2p0",
        .modname = "wlan",
        .supplicant = "rtl_supplicant",
        .p2p_supplicant = "rtl_p2p_wpa",
        .hostapd = "rtl_hostapd",
    },
    {
        .name = "rtl8188cu",
...
struct wifi_info *get_wifi_info()
{
    ...
    for (i = 0; i < sizeof(wifi_infos)/sizeof(wifi_infos[0]); i++) {
        if (strncasecmp(line, wifi_infos[i].name, strlen(wifi_infos[i].name)) == 0) {
            wifi_info_cache = &wifi_infos[i];
            ALOGD("%s: found %s\n", __func__, wifi_infos[i].name);
            return &wifi_infos[i];
        }
    }
    ALOGD("%s: no found [%s]\n", __func__, line);
...
int get_wifi_module_info(char *modpath, char *modarg, char *modname)
{
    struct wifi_info *info = get_wifi_info();
    if (!info)
        return -1;

    snprintf(modpath, SYSFS_PATH_MAX, "%s/%s", MODULE_DEFAULT_DIR, info->modpath);
    snprintf(modarg, PROPERTY_VALUE_MAX, "%s", info->modarg);
    snprintf(modname, PROPERTY_VALUE_MAX, "%s", info->modname);
    return 0;
}
...
int is_wifi_driver_loaded() {
    ...
    if (!is_wifi_driver_builtin()) {
        ...
        get_wifi_module_info(NULL, NULL, driver_modname);
...
int wifi_load_driver()
{
    ...
    if (!is_wifi_driver_builtin()) {
        ...
        get_wifi_module_info(driver_modpath, driver_modarg, NULL);

        ALOGD("%s: %s\n", __func__, driver_modpath);
        insmod(driver_modpath, driver_modarg);
